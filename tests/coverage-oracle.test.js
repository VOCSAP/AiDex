...
import { init, isUnderSystemTemp, LINE_RANGES_SCHEMA } from '../build/commands/init.js';
...
describe('coverage oracle differential', () => {
    ...
    test('a full reindex declares the current schema and stores a MEASURED per-language record', () => {
        const db = new Database(join(dir, '.aidex', 'index.db'), { readonly: true });
        const read = (key) =>
            db.prepare('SELECT value FROM metadata WHERE key = ?').get(key)?.value ?? null;

        expect(read('schema_version')).toBe(LINE_RANGES_SCHEMA);
    ...
    describe('literal dimension on an index that does not declare it', () => {
        ...
        test('init migrates the index instead of skipping unchanged files', async () => {
            ...
            expect(db.prepare("SELECT value v FROM metadata WHERE key='schema_version'").get().v).toBe(LINE_RANGES_SCHEMA);
            db.close();
