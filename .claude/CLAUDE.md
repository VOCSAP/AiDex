...
Dix outils d'administration de l'index et de suivi de session ont suivi (plan `docs/plans/cli-wrappers-hooks-plan.md`, rang 2) : `init`, `global_init`, `coverage`, `settings`, `global_status`, `scan`, `global_refresh`, `viewer`, `remove`, `session`. Chacun pesait 1 a 70 appels agent sur quatre mois et 80 projets. `remove`, `session`, `settings` sans `--open`, `global-status` et `global-refresh` passent par leur handler MCP via `run-tool`. `init`, `rebuild-index`, `scan` et `global-init` partagent leur schema et leur mapping avec leur handler MCP, tout en gardant leurs sorties CLI. `coverage` (sous-commande `can`) et `viewer` gardent leur routage CLI ; les lanceurs `bin/aidex` et `bin/aidex.cmd` resolvent le build depuis leur emplacement, interpreteur `AIDEX_NODE` sinon `node` du PATH ; `settings --open` lance le viewer puis acquitte la version vue. Mesure du 2026-09-14 sur le `tools/list` reellement servi (`registerTools()` du build, un `JSON.stringify` par outil) : 22 outils et 16363 octets avant, 12 outils et 9800 octets apres, soit 6563 octets et 40,1 pourcent de moins, environ 1460 a 1560 tokens par session. `update` et `status` restent annonces (retrait ferme par la mesure : 1790 et 71 appels agent). Les messages qui renvoyaient vers un outil retire donnent desormais une commande CLI executable telle quelle, binaire node du serveur et point d'entree du build entre guillemets (`cliCommand`, `src/commands/shared.ts`) ; `tests/tool-filter.test.js` refuse toute chaine de `src/` qui nomme un outil de `DEFAULT_DISABLED_TOOLS`, hors lignes de commentaire. Un depot git sans index est signale au demarrage de session par le hook `hooks/claude/aidex-init-nudge.py`.
